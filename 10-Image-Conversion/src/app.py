...
from PIL import Image                #! Image is from 'Pillow' and is used to open and process images.
...
def main():
    ...
    st.image(image, caption='Original Image')    # نمایش تصویر اصلی
    ...
    if process_type == "Resize":
        ...
        if st.button("Resize Image"):
            ...
            st.image(resized_image, caption='Resized Image')   # نمایش تصویر جدید
        ...
        mime_type = f"image/{output_format.lower()}"    #  ->   image/jpeg  or  image/png
        if st.button("Convert Image Type"):
            converted_buffer = convert_image_type(image, output_format)          # image = original.jpg
                                                                                  # output_format = PNG
            # نمایش تصویر 
